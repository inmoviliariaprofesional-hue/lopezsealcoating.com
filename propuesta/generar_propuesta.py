...
add_bullet("Correo profesional (ejemplo: info@lopezsealcoating.com) en lugar del Gmail.")
...
add_body(
    "Las siguientes cantidades son de referencia y se confirman al aprobar el alcance. Los montos de "
    "terceros (dominio, correo, presupuesto de anuncios) se pagan directamente a los proveedores.",
...
hdr = inv.rows[0].cells
set_cell_text(hdr[0], "Concepto", bold=True, white=True, size=11)
set_cell_text(hdr[1], "Tipo", bold=True, white=True, size=11, align=WD_ALIGN_PARAGRAPH.CENTER)
set_cell_text(hdr[2], "Inversión (USD)*", bold=True, white=True, size=11, align=WD_ALIGN_PARAGRAPH.CENTER)
for c in hdr:
...
rows = [
    ...
    ("Registro de dominio (1 año)", "Anual", "$15 – $25"),
    ("Correo profesional (1 año)", "Anual", "$40 – $70"),
    ("Perfil de Empresa en Google + optimización", "Único", "$120 – $200"),
    ("Sistema de reseñas (enlace + QR + guion)", "Único", "$60 – $100"),
    ("FASE 1 — Sitio web de conversión", "", ""),
    ("Sitio bilingüe (EN/ES) de conversión", "Único", "$450 – $800"),
    ("Hospedaje del sitio (Cloudflare Pages)", "Anual", "$0 – $60"),
    ("FASE 2 — Crecimiento (opcional)", "", ""),
    ("Gestión de anuncios (mensual)", "Mensual", "$200 – $400"),
    ("Presupuesto de anuncios (se paga a Google/Meta)", "Mensual", "Lo defines tú"),
]
for concepto, tipo, precio in rows:
    row = inv.add_row().cells
    if tipo == "" and precio == "":  # fila de sección
        set_cell_text(row[0], concepto, bold=True, size=10.5)
        set_cell_text(row[1], "", size=10.5)
        set_cell_text(row[2], "", size=10.5)
        for c in row:
            shade_cell(c, AMARILLO_HEX)
    else:
        set_cell_text(row[0], concepto, size=10.5)
        set_cell_text(row[1], tipo, size=10.5, align=WD_ALIGN_PARAGRAPH.CENTER)
        set_cell_text(row[2], precio, size=10.5, align=WD_ALIGN_PARAGRAPH.CENTER)

doc.add_paragraph()
add_body(
    "* Rangos de referencia ajustables por Aimarktech. Recomendación: comenzar con Fase 0 + Fase 1, "
    "que es la base que genera resultados con la inversión más contenida.",
    size=10, color=GRIS,
)
...
set_cell_text(
    cell,
    "Fase 0 + Fase 1 (logo, dominio, correo, Google Business Profile, sistema de reseñas y sitio "
    "bilingüe de conversión). Es el conjunto mínimo que te hace visible y convierte visitas en llamadas.",
    size=10.5,
)
...
add_bullet("Tu teléfono y correo definitivos para el contacto público.")
